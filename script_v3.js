/* ═══════════════════════════════════════════════════════════
      EclesiaSync · JavaScript v3.1
      + Novos tipos de visita/evento (sem oferta/dízimo)
      + Página de Frequência de Usuários
      + Permissão: ver_frequencia_usuarios
   ═══════════════════════════════════════════════════════════ */
...
// ─ TIPOS DE EVENTOS ─────────────────────────────────────────
// Tipos com oferta/dízimo
const TIPOS_COM_FINANCEIRO = ['culto'];
// Tipos com evangelizados
const TIPOS_COM_EVANGELISMO = ['saida', 'ponto_pregacao', 'culto_ar_livre'];
...
function tipoLabel(t) { return TIPOS_EVENTO[t]?.label || t || '—'; }
function tipoIcon(t)  { return TIPOS_EVENTO[t]?.icon  || '📋'; }
function tipoGrupo(t) { return TIPOS_EVENTO[t]?.grupo || 'Outro'; }
function tipoFinanceiro(t)  { return !!TIPOS_EVENTO[t]?.financeiro; }
function tipoEvangelismo(t) { return !!TIPOS_EVENTO[t]?.evangelismo; }
...
const PERM_DESC = {
  ...
  'ver_frequencia_usuarios':    { label: 'Ver Frequência de Usuários',     desc: 'Ver frequência de participação em cultos/eventos por usuário do setor' },
};
  ...
  try {
    ...
    if (data && !error) {
      ...
      const map = {
        ...
        'Ver Frequência de Usuários': 'ver_frequencia_usuarios',
      };
      (legado || []).forEach(p => {
    ...
    console.warn('Permissões via RPC indisponíveis:', e);
  }
}
...
function navigate(page) {
  ...
  const titles = {
    dashboard: 'Dashboard', setores: 'Setores', usuarios: 'Usuários',
    relatorios: 'Relatórios', permissoes: 'Permissões', frequencia: 'Frequência de Usuários'
  };
...
async function renderDashboard() {
  ...
  if (!canSeeAllSetores() && currentUser?.setor_id) qAg = qAg.eq('setor_id', currentUser.setor_id);
...
function tipoColor(t) {
  const colors = {
    culto:'var(--gold)', evento:'var(--blue)', saida:'var(--teal)',
    visita_enfermos:'#f59e0b', visita_desviados:'#ec4899', visita_detidos:'#ef4444',
    visita_convertidos:'#14b8a6', visita_umadalpe:'#8b5cf6', visita_recebida_umadalpe:'#6366f1',
    almas_salvas:'#fbbf24', batismo_espirito:'#38bdf8', renovo:'#4ade80',
    culto_ar_livre:'#fb923c', ponto_pregacao:'#a78bfa',
  };
...
async function openEditCongModal(id) {
  ...
  <div class="form-group"><label>Dirigente(s)</label><select id="ec-dirigente" multiple style="height:90px">${userOptions}</select><small class="c3 fs-xs">Segure Ctrl para selecionar múltiplos</small></div>
...
async function saveCong(id) {
  ...
  const payload = { nome, endereco:($('ec-end')?.value||'').trim()||null, pastor_local:($('ec-pastor')?.value||'').trim()||null,
    latitude:parseFloat($('ec-lat')?.value)||null, longitude:parseFloat($('ec-lng')?.value)||null,
    dirigente:getSelected('ec-dirigente')||null, vice_dirigente:getSelected('ec-vice')||null, secretaria:getSelected('ec-sec')||null };
...
async function renderCongregacao(pc) {
  ...
  const { data: agendaSemana } = await q('agenda_semana').select('*').eq('congregacao_id',c.id)
  ...
  <div class="sec-hdr">
    ...
    <div class="sec-actions">
      ...
      <div class="dropdown-wrap" style="position:relative">
        ...
        <div id="event-menu" class="dropdown-menu hidden">${buildEventMenuHtml()}</div>
      </div>`:''}
    </div>
  </div>
    ...
    ${(eventos||[]).map(e=>`
      <div class="act-item" onclick="openEventDetail('${e.id}')" style="cursor:pointer">
        ...
        <div style="text-align:right">
          <span class="tag">${e.participantes||0} pessoas</span>
          ${tipoFinanceiro(e.tipo)?`<div class="fs-xs c3 mt8">${fmtMoney(e.ofertas||0)} + ${fmtMoney(e.dizimos||0)}</div>`:''}
        </div>
        <span class="act-time">${fmtDate(e.data)}</span>
...
function buildEventMenuHtml() {
  ...
  Object.entries(grupos).forEach(([grupo, itens]) => {
    html += `<div class="dropdown-label">${grupo}</div>`;
    itens.forEach(({ tipo, label, icon }) => {
      html += `<div class="dropdown-item" onclick="openEventModal('${tipo}')">${icon} ${label}</div>`;
    });
  });
  return html;
}
...
function buildMapLinks(c) {
  ...
  return `<span class="map-links">
    <a href="${gmaps}" target="_blank" rel="noopener" class="map-btn maps-btn">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>Maps</a>
    <a href="${waze}" target="_blank" rel="noopener" class="map-btn waze-btn">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.54 6.63C19.3 3.88 16.6 2 13.5 2 9.36 2 6 5.36 6 9.5c0 .66.08 1.3.24 1.92l-1.87 1.88c-.59.59-.59 1.54 0 2.13L6 17.06V19c0 1.1.9 2 2 2h2v-2H8v-2.41l-1.29-1.3 2.44-2.44C10.07 13.58 11.24 14 12.5 14c2.49 0 4.5-2.01 4.5-4.5 0-.34-.04-.67-.1-1l2.44 2.44-1.3 1.3V14h-2v2h2c1.1 0 2-.9 2-2v-1.94l1.63-1.63c.59-.59.59-1.54 0-2.13l-1.13-1.67z"/></svg>Waze</a>
  </span>`;
}
...
function renderAgendaSemanaGrid(items, inicioSemana, congId) {
  ...
  for (let d=0;d<7;d++) {
    ...
    html+=`<div class="agenda-day${isToday?' agenda-today':''}">
      <div class="ag-day-head"><span class="ag-day-name">${dias[d]}</span><span class="ag-day-num">${dia.getDate()}</span></div>
...
async function saveAgenda(congId, editId) {
  ...
  const titulo=($('ag-titulo')?.value||'').trim(); const data=$('ag-data')?.value;
...
async function openEventModal(tipo) {
  ...
  if (info.financeiro) {
    ...
    // Visitas, resultados espirituais, jovens — só quantidade
    extraFields=`
    <div class="form-row">
      <div class="form-group"><label>Quantidade / Participantes</label><input id="ev-participantes" type="number" min="0" placeholder="0"/></div>
      <div class="form-group"><label>Data</label></div>
    </div>`;
  }
  ...
  <div class="modal-hdr">
    ...
    <h2>Registrar: ${info.label}</h2>
    <button class="modal-close" onclick="closeModal()">✕</button>
  </div>
  <div class="modal-body">
    <div class="form-group"><label>Data *</label><input id="ev-data" type="date" value="${new Date().toISOString().slice(0,10)}"/></div>
    <div class="form-group"><label>Resumo / Observações</label><textarea id="ev-resumo" rows="2" placeholder="Breve descrição..." style="resize:vertical"></textarea></div>
    ${extraFields}
...
async function openEventDetail(id) {
  ...
  if (info.financeiro) {
    ...
    detalhes=`<div class="mem-info-grid"><div class="inf-item"><label>Quantidade / Participantes</label><span>${ev.participantes||0}</span></div></div>`;
  }
  showModal(`
  <div class="mem-profile">
    <button class="modal-close" style="position:absolute;top:14px;right:14px" onclick="closeModal()">✕</button>
    <div style="font-size:40px;margin-bottom:8px">${info.icon}</div>
    <div class="mem-modal-name">${info.label}</div>
...
async function submitAdd(type) {
  ...
  if (type==='setor') {
    ...
    if (!hasPerm('gerenciar_congregacoes')) { toast('Sem permissão','error'); return; }
    ({error}=await q('congregacoes').insert({nome,setor_id:navState.setor.id,endereco:$('add-end')?.value||null,pastor_local:$('add-past')?.value||null,latitude:parseFloat($('add-lat')?.value)||null,longitude:parseFloat($('add-lng')?.value)||null}));
  } else {
    if (!hasPerm('gerenciar_membros')) { toast('Sem permissão','error'); return; }
    ({error}=await q('membros').insert({nome,congregacao_id:navState.cong.id,setor_id:navState.setor.id,cargo:$('add-cargo').value,idade:parseInt($('add-idade')?.value)||null,telefone:$('add-tel')?.value||null,email:$('add-email')?.value||null}));
  }
  if (error) { toast(error.message,'error'); return; }
...
function openUserModal(id) {
  ...
  if (id) {
    Promise.all([q('sistema_usuarios').select('*').eq('id',id).single(),q('setores').select('id,nome').order('nome')])
    ...
    q('setores').select('id,nome').order('nome').then(({data:setores})=>{ $('user-modal-body').innerHTML=userFormHtml(null,ROLES,setores||[]); });
  }
...
async function saveUser(id) {
  const nome=($('um-name')?.value||'').trim(); const username=($('um-username')?.value||'').trim();
  const senha=($('um-pass')?.value||'').trim(); const setorId=$('um-setor')?.value||null;
  if (!nome||!username) { toast('Nome e username são obrigatórios','error'); return; }
  if (!id&&!senha) { toast('Senha é obrigatória','error'); return; }
  const payload={ nome, username, role:$('um-role').value, cargo:$('um-cargo').value, congregacao:$('um-cong').value, idade:parseInt($('um-age')?.value)||null, ativo:$('um-ativo').value==='true', setor_id:setorId||null };
...
async function openUserPermModal(userId, userName) {
  ...
  ${Object.entries(PERM_DESC).map(([code,{label,desc}])=>{
    const on=!!resolved[code]; const isOverride=userOverrides.hasOwnProperty(code);
...
async function toggleUserPerm(userId, perm, current) {
  ...
  try {
    ...
    const { error }=await q('user_permissions').upsert({user_id:userId,permission_code:perm,ativo:novoValor},{onConflict:'user_id,permission_code'});
    if (error) { toast(error.message,'error'); return; }
  }
...
async function renderFrequencia() {
  if (!hasPerm('ver_frequencia_usuarios')) {
    $('page-content').innerHTML=`<div class="empty"><div class="empty-ico">🔐</div><p>Você não tem permissão para acessar frequência de usuários.</p></div>`; return;
  }
  $('page-content').innerHTML=loadingPage();
  ...
  let qEventos=q('eventos').select('id,tipo,data,participante_ids,setor_id,congregacao_id').gte('data',freqFiltroInicio).lte('data',freqFiltroFim);
  const { data: setores }=await q('setores').select('id,nome').order('nome');

  // Filtro por setor
  if (!canSeeAllSetores()&&currentUser?.setor_id) {
    qUsuarios=qUsuarios.eq('setor_id',currentUser.setor_id);
    qEventos=qEventos.eq('setor_id',currentUser.setor_id);
  } else if (freqSetorFiltro) {
    qUsuarios=qUsuarios.eq('setor_id',freqSetorFiltro);
    qEventos=qEventos.eq('setor_id',freqSetorFiltro);
  }
  ...
  const totalCultos=eventosList.filter(e=>e.tipo==='culto').length;
  ...
  const freqData=usuariosList.map(u=>{
    ...
    const totalParticipou=evParticipou.length;
    const pctTotal=totalEventos>0?Math.round((totalParticipou/totalEventos)*100):0;
    ...
    return { ...u, evParticipou, cultosParticipou, totalParticipou, pctTotal, pctCultos, setorNome };
  }).sort((a,b)=>b.pctTotal-a.pctTotal);

  const setorOpcoes=canSeeAllSetores()
    ? `<option value="">Todos os setores</option>${(setores||[]).map(s=>`<option value="${s.id}" ${s.id===freqSetorFiltro?'selected':''}>${escHtml(s.nome)}</option>`).join('')}`
    : '';
  ...
  <div class="sec-hdr">
    ...
    <div class="sec-actions">
      ${hasPerm('exportar_dados')?`<button class="btn btn-primary btn-sm" onclick="exportarFrequenciaPDF()">📄 Exportar PDF</button>`:''}
    </div>
  </div>

  <div class="filter-bar">
    <div class="filter-title">📅 Filtro de Período</div>
    <div class="filter-fields">
      ${canSeeAllSetores()?`<div class="form-group" style="margin:0"><label>Setor</label><select id="freq-setor" onchange="freqSetorFiltro=this.value">${setorOpcoes}</select></div>`:''}
      <div class="form-group" style="margin:0"><label>Data Inicial</label><input type="date" id="freq-inicio" value="${freqFiltroInicio}" onchange="freqFiltroInicio=this.value"/></div>
      <div class="form-group" style="margin:0"><label>Data Final</label><input type="date" id="freq-fim" value="${freqFiltroFim}" onchange="freqFiltroFim=this.value"/></div>
      <div style="display:flex;gap:8px;align-items:flex-end">
        <button class="btn btn-primary btn-sm" onclick="${canSeeAllSetores()?'freqSetorFiltro=$("freq-setor")?.value||"";':''}renderFrequencia()">🔍 Filtrar</button>
        <button class="btn btn-secondary btn-sm" onclick="freqFiltroInicio='';freqFiltroFim='';freqSetorFiltro='';renderFrequencia()">↺ Limpar</button>
      </div>
    </div>
  ...
  <div class="stats-grid stats-4" style="margin-bottom:24px">
    ${statCard('📋','ic-gold',totalEventos,'Total de Eventos','no período')}
    ${statCard('⛪','ic-blue',totalCultos,'Cultos','no período')}
    ${statCard('👥','ic-teal',usuariosList.length,'Usuários','monitorados')}
    ${statCard('📈','ic-violet',freqData.length>0?`${freqData[0]?.pctTotal||0}%`:'—','Maior frequência',freqData[0]?.nome?.split(' ')[0]||'')}
  </div>

  ${!canSeeAllSetores()&&!isSuperAdmin()?`<div class="access-notice">🔒 Exibindo apenas usuários do seu setor: <strong>${escHtml(currentUserSetor?.nome||'Seu setor')}</strong></div>`:''}

  <div class="freq-legend" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;font-size:.75rem;color:var(--txt2)">
    <span>Legenda:</span>
    <span><span class="freq-dot" style="background:#14b8a6"></span> ≥75% Alta</span>
    <span><span class="freq-dot" style="background:#f59e0b"></span> 50–74% Média</span>
    <span><span class="freq-dot" style="background:#f43f5e"></span> &lt;50% Baixa</span>
  </div>
      ...
      <tbody>
        ${freqData.length?freqData.map(u=>{
          ...
          return `<tr>
            <td data-label="Usuário"><div class="flex items-center gap8">
              <div class="av av-sm" style="background:${avatarColor(u.nome)}">${initials(u.nome)}</div>
              <div><div class="fw5 fs-sm">${escHtml(u.nome)}</div><div class="fs-xs c3">${escHtml(u.congregacao||'—')}</div></div>
            </div></td>
            <td data-label="Cargo" class="fs-sm">${escHtml(u.cargo||'—')}</td>
            ${canSeeAllSetores()?`<td data-label="Setor" class="fs-sm c2">${escHtml(u.setorNome)}</td>`:''}
            <td data-label="Freq. Geral">
              <div style="display:flex;align-items:center;gap:8px">
                <div class="freq-bar-wrap"><div class="freq-bar" style="width:${u.pctTotal}%;background:${corGeral}"></div></div>
                <span class="fw6 fs-sm" style="color:${corGeral};min-width:36px">${u.pctTotal}%</span>
              </div>
            </td>
            <td data-label="Freq. Cultos">
              <div style="display:flex;align-items:center;gap:8px">
                <div class="freq-bar-wrap"><div class="freq-bar" style="width:${u.pctCultos}%;background:${corCultos}"></div></div>
                <span class="fw6 fs-sm" style="color:${corCultos};min-width:36px">${u.pctCultos}%</span>
              </div>
            </td>
            <td data-label="Eventos Part." class="fs-sm" style="text-align:center">${u.totalParticipou}/${totalEventos}</td>
            <td data-label="Cultos Part." class="fs-sm" style="text-align:center">${u.cultosParticipou}/${totalCultos}</td>
            <td class="td-no-label"><button class="btn btn-secondary btn-sm" onclick="openFreqDetalhe('${u.id}','${escHtml(u.nome)}')">Ver →</button></td>
          </tr>`;
        }).join(''):`<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--txt3)">Nenhum dado encontrado para o período.</td></tr>`}
      </tbody>
    </table>
  </div></div>
  ...
  <div class="chart-card" style="margin-bottom:28px">
    <h3>Top Usuários por Frequência Geral</h3><p>No período selecionado</p>
    <canvas id="chart-freq" height="80"></canvas>
  </div>`;

  // Chart
  const top10=freqData.slice(0,10);
  const fCtx=document.getElementById('chart-freq');
  if (fCtx&&top10.length) {
    chartInstances.freq=new Chart(fCtx,{
      type:'bar',
      data:{
        labels:top10.map(u=>u.nome.split(' ')[0]),
        datasets:[
          { label:'Freq. Geral (%)', data:top10.map(u=>u.pctTotal), backgroundColor:top10.map(u=>u.pctTotal>=75?'rgba(20,184,166,.8)':u.pctTotal>=50?'rgba(245,158,11,.8)':'rgba(244,63,94,.8)'), borderRadius:8 },
          { label:'Freq. Cultos (%)', data:top10.map(u=>u.pctCultos), backgroundColor:'rgba(201,168,76,.4)', borderRadius:8 },
        ]
      },
      options:{
        responsive:true,
        plugins:{ legend:{ labels:{ color:'#94a3b8' } } },
        scales:{ x:{ ticks:{ color:'#94a3b8' }, grid:{ color:'rgba(255,255,255,.03)' } }, y:{ min:0, max:100, ticks:{ color:'#94a3b8', callback:v=>v+'%' }, grid:{ color:'rgba(255,255,255,.05)' } } }
      }
    });
  }
}
...
async function openFreqDetalhe(userId, userName) {
  showModal(`
  <div class="modal-hdr"><span>📈</span><h2>Frequência — ${escHtml(userName)}</h2><button class="modal-close" onclick="closeModal()">✕</button></div>
  <div class="modal-body" id="freq-detalhe-body"><div class="loading-page"><div class="spinner"></div></div></div>`);

  const { data: eventos }=await q('eventos').select('id,tipo,data,resumo,congregacao_id').gte('data',freqFiltroInicio).lte('data',freqFiltroFim);
  const eventosDoUsuario=(eventos||[]).filter(e=>(e.participante_ids||[]).includes(userId));
  // Buscar congregações para nomes
  const congIds=[...new Set((eventos||[]).map(e=>e.congregacao_id).filter(Boolean))];
  ...
  const partEv=eventosDoUsuario.length;
  ...
  <div style="border-bottom:1px solid var(--bdr2);margin-bottom:14px;padding-bottom:10px">
    <div class="fs-xs c3 fw6" style="text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Eventos que participou</div>
    ${eventosDoUsuario.length?eventosDoUsuario.map(e=>`
      <div class="act-item" style="margin-bottom:6px">
        <div class="act-dot" style="background:${tipoColor(e.tipo)}"></div>
        <div class="f1">
          <div class="fw5 fs-sm">${tipoIcon(e.tipo)} ${tipoLabel(e.tipo)}</div>
          <div class="fs-xs c3">${escHtml(e.resumo||'')} · ${escHtml(congNome(e.congregacao_id))}</div>
        </div>
        <span class="act-time">${fmtDate(e.data)}</span>
      </div>`).join('')
    :'<p class="c3 fs-sm" style="text-align:center;padding:12px">Nenhum evento registrado para este usuário.</p>'}
  </div>
  ${(eventos||[]).filter(e=>!(e.participante_ids||[]).includes(userId)).length?`
  <div class="fs-xs c3 fw6" style="text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Ausências</div>
  ${(eventos||[]).filter(e=>!(e.participante_ids||[]).includes(userId)).map(e=>`
    <div class="act-item" style="margin-bottom:6px;opacity:.5">
      <div class="act-dot" style="background:#475569"></div>
      <div class="f1"><div class="fw5 fs-sm">${tipoIcon(e.tipo)} ${tipoLabel(e.tipo)}</div><div class="fs-xs c3">${escHtml(congNome(e.congregacao_id))}</div></div>
      <span class="act-time">${fmtDate(e.data)}</span>
    </div>`).join('')}`:''}`;
}
...
async function exportarFrequenciaPDF() {
  ...
  const { jsPDF }=window.jspdf; if (!jsPDF) { toast('Biblioteca PDF não carregada','error'); return; }
  toast('Gerando PDF...','info');

  let qU=q('sistema_usuarios').select('id,nome,role,cargo,setor_id,congregacao,ativo').eq('ativo',true).order('nome');
  let qE=q('eventos').select('id,tipo,data,participante_ids,setor_id').gte('data',freqFiltroInicio).lte('data',freqFiltroFim);
  if (!canSeeAllSetores()&&currentUser?.setor_id) { qU=qU.eq('setor_id',currentUser.setor_id); qE=qE.eq('setor_id',currentUser.setor_id); }
  else if (freqSetorFiltro) { qU=qU.eq('setor_id',freqSetorFiltro); qE=qE.eq('setor_id',freqSetorFiltro); }
  const [{ data: usuarios },{ data: eventos }]=await Promise.all([qU,qE]);
  const { data: setores }=await q('setores').select('id,nome');
  const totalEv=(eventos||[]).length, totalCultos=(eventos||[]).filter(e=>e.tipo==='culto').length;
  const freqData=(usuarios||[]).map(u=>{
    const part=(eventos||[]).filter(e=>(e.participante_ids||[]).includes(u.id));
    const cultosPart=part.filter(e=>e.tipo==='culto').length;
    const pctTotal=totalEv>0?Math.round((part.length/totalEv)*100):0;
    const pctCultos=totalCultos>0?Math.round((cultosPart/totalCultos)*100):0;
    const setorNome=(setores||[]).find(s=>s.id===u.setor_id)?.nome||'—';
    return { nome:u.nome, cargo:u.cargo||'—', setorNome, congregacao:u.congregacao||'—', partTotal:part.length, cultosPart, pctTotal, pctCultos };
  }).sort((a,b)=>b.pctTotal-a.pctTotal);
  ...
  doc.text('Frequência por Usuário',margin,y); y+=8;
  doc.autoTable({
    startY:y, margin:{left:margin,right:margin},
    head:[['Usuário','Cargo','Setor','Freq. Geral','Freq. Cultos','Part./Total','Cultos/Total']],
    body:freqData.map(u=>[u.nome,u.cargo,u.setorNome,`${u.pctTotal}%`,`${u.pctCultos}%`,`${u.partTotal}/${totalEv}`,`${u.cultosPart}/${totalCultos}`]),
    theme:'grid',
    ...
    didParseCell: function(data) {
      if (data.section==='body'&&data.column.index===3) {
        ...
        if (pct>=75) data.cell.styles.textColor=[20,184,166];
...
async function renderRelatorios() {
  ...
  if (!canSeeAllSetores()&&currentUser?.setor_id) {
    const sid=currentUser.setor_id;
    qEv=qEv.eq('setor_id',sid); qCong=qCong.eq('setor_id',sid); qSet=qSet.eq('id',sid); qMem=qMem.eq('setor_id',sid);
  }
      ...
      <tbody>
        ${eventos.map(e=>{
          ...
          return `<tr>
            ...
            <td data-label="Tipo"><span class="tag" style="background:${tipoColor(e.tipo)}22;color:${tipoColor(e.tipo)}">${tipoIcon(e.tipo)} ${tipoLabel(e.tipo)}</span></td>
        ...
        }).join('')||'<tr><td colspan="8" style="text-align:center;color:var(--txt3);padding:20px">Nenhum evento registrado neste período</td></tr>'}
      </tbody>
    </table>
  </div></div>`;
...
async function exportarPDF() {
  ...
  if (!canSeeAllSetores()&&currentUser?.setor_id) { const sid=currentUser.setor_id; qEv=qEv.eq('setor_id',sid); qCong=qCong.eq('setor_id',sid); qSet=qSet.eq('id',sid); qMem=qMem.eq('setor_id',sid); }
  const [rEv,rCong,rSet,rMem]=await Promise.all([qEv,qCong,qSet,qMem]);
  const eventos=rEv.data||[],congs=rCong.data||[],setores=rSet.data||[];
  const memCount=id=>(rMem.data||[]).filter(m=>m.congregacao_id===id).length;
  const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
  ...
  doc.autoTable({ startY:y, margin:{left:margin,right:margin}, head:[['Indicador','Valor']],
    body:[['Total de Setores',setores.length],['Total de Congregações',congs.length],['Total de Membros',(rMem.data||[]).length],['Total de Eventos',eventos.length],['Cultos',eventos.filter(e=>e.tipo==='culto').length],['Eventos Genéricos',eventos.filter(e=>e.tipo==='evento').length],['Saídas Evangelísticas',eventos.filter(e=>e.tipo==='saida').length],['Outros Registros',eventos.filter(e=>!['culto','evento','saida'].includes(e.tipo)).length],['Total de Participantes',totalPart],['Total de Conversões',totalConv],['Total de Ofertas',fmtMoney(totalOfer)],['Total de Dízimos',fmtMoney(totalDiz)],['Total Arrecadado',fmtMoney(totalOfer+totalDiz)]],
    theme:'grid', headStyles:{fillColor:[9,12,24],textColor:[201,168,76],fontStyle:'bold'}, alternateRowStyles:{fillColor:[245,245,250]}, styles:{fontSize:9} });
  ...
  for (const s of setores) {
    ...
    doc.text(`Setor: ${s.nome} (Região ${s.regiao||'—'})`,margin+2,y+2); y+=12;
    const sCongs=congs.filter(c=>c.setor_id===s.id);
    ...
    for (const c of sCongs) {
      ...
      y+=12;
      if (cEvs.length) {
        doc.autoTable({ startY:y, margin:{left:margin+6,right:margin},
          ...
          theme:'striped', headStyles:{fillColor:[30,30,50],textColor:[201,168,76],fontSize:7,fontStyle:'bold'}, styles:{fontSize:7.5},
          columnStyles:{0:{cellWidth:20},1:{cellWidth:30},2:{cellWidth:42},3:{cellWidth:12},4:{cellWidth:12},5:{cellWidth:24},6:{cellWidth:24}} });
...
async function renderPermissoes() {
  ...
  const grupos={
    'Acesso Geral': ['visualizar_dashboard','ver_relatorios','ver_frequencia_usuarios','exportar_dados'],
    ...
    'Visibilidade': ['ver_todos_setores'],
  ...
  <div style="background:rgba(201,168,76,.07);border:1px solid rgba(201,168,76,.2);border-radius:10px;padding:12px 16px;margin-bottom:20px;font-size:.82rem;color:var(--txt2)">
    ...
    <br>🔒 <strong>Ver Todos os Setores</strong> — somente leitura para outros setores.
    <br>📈 <strong>Ver Frequência de Usuários</strong> — nova permissão controlada via banco de dados.
    <br>🔐 Use o botão <strong>Permissões Individuais</strong> na lista de usuários para sobrescrever por usuário.
  </div>
  <div class="role-tabs">
    ${ROLES.map(r=>`<button class="btn ${r===activeRole?'btn-primary':'btn-secondary'} btn-sm" onclick="setActiveRole('${r}')"><span class="role-badge ${roleCls(r)}">${r}</span></button>`).join('')}
  </div>
  <div class="tbl-wrap" style="max-width:680px">
    ...
    <div style="padding:6px 18px">
      ${Object.entries(grupos).map(([grupo, codes])=>`
        <div style="font-size:.68rem;font-weight:700;color:var(--txt3);letter-spacing:.1em;text-transform:uppercase;padding:14px 0 6px;border-bottom:1px solid var(--bdr2)">${grupo}</div>
        ${codes.map(perm=>{
          const info=PERM_DESC[perm]; if (!info) return '';
          const on=!!displayPerms[perm];
          const isAdminRole=activeRole==='admin';
          const isNewPerm=perm==='ver_frequencia_usuarios';
          return `<div class="perm-row">
            <div class="perm-lbl">
              <strong>${info.label}${isNewPerm?' <span class="tag tag-blue" style="font-size:.6rem">novo</span>':''}</strong>
              <span>${info.desc}</span>
            </div>
...
function closeModal() {
  const mc=$('modal-container'); const ov=mc.querySelector('.overlay');
...
(async function() {
  try {
    ...
    if (saved) { currentUser=saved; await loadPermissions(); await loadUserSetor(); startApp(saved); }
  } catch(e) {}
  $('inp-user').focus();
})();
